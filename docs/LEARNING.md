...
**The Kafka-engine → MV → MergeTree pattern.** A ClickHouse table with `ENGINE = Kafka` isn't storage at all — it's a *view onto a live topic*. Querying it directly gives you whatever's currently in flight and then it's gone; nothing is persisted. The trick is attaching one or more **materialized views** to it: each time ClickHouse polls a batch of messages off the topic, it runs every attached MV's `SELECT` against that batch and writes the result into the MV's target table. That's the entire ingestion pipeline: `posts_queue` (Kafka engine) → `posts_queue_to_posts_mv` (raw copy) + `posts_queue_to_per_minute_mv` (rollup) + `posts_queue_to_tags_mv` (per-tag rollup), all three firing off the exact same consumed batch, with no separate stream-processing framework involved.
